////
// rename some items to fit the pack
//// renik
// script originally created by ArugChief in the KubeJS Discord.

//
ClientEvents.lang('en_us', event => {
//
  const rename = (item, newName) => {
	event.renameItem(item, newName)}

// OCEAN RECOVERY
  	rename('ocean_recovery:plastic_shard', "Plastic Scrap")

// ICE AND FIRE
  	rename('iceandfire:silver_ore', "Silver Ore 1")
  	rename('iceandfire:deepslate_silver_ore', "Deepslate Silver Ore 1")

// OCCULTISM
  	rename('occultism:silver_ore', "Silver Ore 2")
  	rename('occultism:silver_ore_deepslate', "Deepslate Silver Ore 2")



//
})
//